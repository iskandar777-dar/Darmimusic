...
from Darmimusic import Telegram, YouTube, app
from Darmimusic.misc import SUDOERS
from Darmimusic.plugins.play.playlist import del_plist_msg
from Darmimusic.plugins.sudo.sudoers import sudoers_list
from Darmimusic.utils.database import (add_served_chat,
                                       add_served_user,
                                       blacklisted_chats,
                                       get_assistant, get_lang,
                                       get_userss, is_on_off,
                                       is_served_private_chat)
from Darmimusic.utils.decorators.language import LanguageStart
from Darmimusic.utils.inline import (help_pannel, private_panel,
                                     start_pannel)
        ...
        if name[0:3] == "sud":
            ...
            if await is_on_off(config.LOG):
                ...
                return await app.send_message(
                    config.LOG_GROUP_ID,
                    f"{message.from_user.mention} Mengambil statistik pribadi Anda <code>SUDOLIS</code>\n\n**IDENTITAS PENGGUNA:** {sender_id}\n**NAMA BELAKANG:** {sender_name}",
                )
            return
        if name[0:3] == "lyr":
...
⚡️ __Searched Powered By {config.MUSIC_BOT_NAME}__"""
            ...
            if await is_on_off(config.LOG):
                ...
                return await app.send_message(
                    config.LOG_GROUP_ID,
                    f"{message.from_user.mention} baru saja mulai bot untuk memeriksa <code>INFORMASI VIDEO</code>\n\n**IDENTITAS PENGGUNA :** {sender_id}\n**NAMA BELAKANG :** {sender_name}",
                )
    else:
        try:
