...
from Darmimusic.utils import (Yukkibin, get_channeplayCB,
                              seconds_to_min)
from Darmimusic.utils.database import (get_cmode, is_active_chat,
                                       is_music_playing)
from Darmimusic.utils.decorators.language import language, languageCB
from Darmimusic.utils.inline import queue_back_markup, queue_markup
...
async def ping_com(client, message: Message, _):
    ...
    if "live_" in file:
        ...
        if videoid == "telegram":
            ...
            IMAGE = get_image(videoid)
    send = (
        "**⌛️Durasi:** Streaming Durasi Tidak Dikenal\n\nKlik tombol di bawah untuk mendapatkan seluruh daftar antrean."
        if DUR == "Unknown"
        else "\nKlik tombol di bawah untuk mendapatkan seluruh daftar antrean."
    )
...
async def queue_back(client, CallbackQuery: CallbackQuery, _):
    ...
    cap = f"""**{config.MUSIC_BOT_NAME} Pemain**
