...
from Darmimusic.misc import HAPP, SUDOERS, XCB
from Darmimusic.utils.database import (get_active_chats,
                                       remove_active_chat,
                                       remove_active_video_chat)
from Darmimusic.utils.decorators.language import language
from Darmimusic.utils.pastebin import Yukkibin
...
async def update_(client, message, _):
    ...
    for info in repo.iter_commits(
        ...
        updates += f"<b>➣ #{info.count()}: [{info.summary}]({REPO_}/commit/{info}) Oleh -> {info.author}</b>\n\t\t\t\t<b>➥ Berkomitmen:</b> {ordinal(int(datetime.fromtimestamp(info.committed_date).strftime('%d')))} {datetime.fromtimestamp(info.committed_date).strftime('%b')}, {datetime.fromtimestamp(info.committed_date).strftime('%Y')}\n\n"
        ...
        try:
            ...
            for x in served_chats:
                try:
                    ...
                    pass
            await response.edit(
                f"{nrs.text}\n\nBot berhasil diperbarui di Heroku! Sekarang, tunggu 2 - 3 menit hingga bot dimulai ulang!"
            )
            os.system(
            ...
            await response.edit(
                f"{nrs.text}\n\nTerjadi kesalahan saat memulai reboot! Coba lagi nanti atau periksa log untuk info lebih lanjut."
            )
            return await app.send_message(
                config.LOG_GROUP_ID,
                f"PENGECUALIAN TERJADI DI #UPDATER DUE TO: <code>{err}</code>",
            )
    else:
        served_chats = await get_active_chats()
        for x in served_chats:
            try:
                ...
                pass
        await response.edit(
            f"{nrs.text}\n\nBot berhasil diperbarui! Sekarang, tunggu 1 - 2 menit hingga bot melakukan boot ulang!"
        )
...
async def restart_(_, message):
    ...
    try:
        ...
        pass
    await response.edit(
        "Reboot telah berhasil dimulai! Tunggu 1 - 2 menit hingga bot dimulai ulang."
    )
    os.system(f"kill -9 {os.getpid()} && bash start")
