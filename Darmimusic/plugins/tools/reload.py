...
from Darmimusic.utils.database import get_authuser_names, get_cmode
from Darmimusic.utils.decorators import (ActualAdminCB, AdminActual,
                                         language)
from Darmimusic.utils.formatters import alpha_to_int
...
async def restartbot(client, message: Message, _):
    ...
    return await mystic.edit_text(
        "Berhasil dimulai ulang. Coba mainkan sekarang.."
    )
...
async def stop_download(client, CallbackQuery: CallbackQuery, _):
    ...
    if task.done() or task.cancelled():
        return await CallbackQuery.answer(
            "Pengunduhan sudah Selesai atau Dibatalkan.",
            show_alert=True,
        )
    if not task.done():
        try:
